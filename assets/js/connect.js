...
(function () {
    ...
    Hoplix_Connect = {
        ...
        loader: function () {
            jQuery('.hoplix-connect-button').click(function () {
                jQuery(this).hide();
                jQuery(this).siblings('.loader').removeClass('hidden');

                setTimeout(function() {
                    Hoplix_Connect.hide_loader();
                }, 60000); //hide the loader after a minute, assume failure
            });
        },
        hide_loader: function() {
            var button = jQuery('.hoplix-connect-button');
            button.show();
            ...
            jQuery.ajax( {
                ...
                success: function( response ) {
                    if (response === 'OK') {
                        ...
                        Hoplix_Connect.send_return_message();
                    }
                }
            });
        },
        listen_auth_return: function () {
            ...
            intercom.on('hoplix-auth', function (data) {
        ...
        send_return_message: function () {
            var intercom = Intercom.getInstance();
            intercom.emit('hoplix-auth', {success: true});
            window.top.close();
        }
    };
})();